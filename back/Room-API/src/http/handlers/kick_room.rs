...
use crate::types::{KickRoomRequest, ServiceRequest};
...
pub async fn handle_kick_room(
    ...
    let kick_req: KickRoomRequest = match serde_json::from_str(&request.body) {
        Ok(req) => req,
        Err(_) => return json_error(400, "Invalid request body"),
    };

    let target_id = match uuid::Uuid::parse_str(&kick_req.user_id) {
        Ok(id) => id,
        Err(_) => return json_error(400, "Invalid user id"),
    };

    match room_service::kick_player(
        redis_pool,
        &session_manager,
        notification_bus,
        &kick_req.room_id,
