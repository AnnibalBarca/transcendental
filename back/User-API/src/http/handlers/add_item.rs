...
pub async fn handle_add_item(ctx: &AppContext, request: &ServiceRequest) -> serde_json::Value {
    ...
    match shop::purchase(ctx.db.get_pool(), &user_id, &body.item_id, &body.item_type).await {
        Ok(new_wallet) => {
        ...
        Err(e) => {
            error!("[Shop] purchase failed: {}", e);
            match e.as_str() {
                "item not available" => json_error(404, "Item not available"),
                "already owned" => json_error(409, "You already own this item"),
