...
pub async fn handle_cancel_friend_request(
    db: &Database,
    conn: &mut Connection,
    _redis_pool: &Pool,
    ...
    let claims = match validate_access_token(conn, token).await {
        Ok(claims) => claims,
        Err(e) => {
            tracing::warn!("[User] Failed to validate token for cancel friend: {}", e);
    ...
    match friend::cancel_request(db.get_pool(), notification_bus, &user_id, &friend_id).await {
        Ok(_) => json!({
            "status": 200,
            "message": "Friend request cancelled"
        }),
        Err(e) => {
            error!("[User] Failed to cancel friend request: {}", e);
            json_error(500, "Failed to cancel friend request")
        }
    }
}
