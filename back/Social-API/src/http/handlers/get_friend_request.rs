...
pub async fn handle_get_friend_requests(
    ...
    let claims = match validate_access_token(conn, token).await {
        ...
        Err(e) => {
            tracing::warn!(
                "[User] Failed to validate token for get friend requests: {}",
                e
            );
    ...
    match friend::get_pending_requests(db.get_pool(), redis_pool, &user_id).await {
        Ok(requests) => json!({
            "status": 200,
            "requests": requests
        }),
        Err(e) => {
            error!("[User] Failed to get friend requests: {}", e);
            json_error(500, "Failed to get friend requests")
        }
    }
}
