...
pub async fn handle_refuse_friend_request(
    db: &Database,
    conn: &mut Connection,
    _redis_pool: &Pool,
    ...
    let claims = match validate_access_token(conn, token).await {
        Ok(claims) => claims,
        Err(e) => {
            tracing::warn!("[User] Failed to validate token for refuse friend: {}", e);
    ...
    match friend::refuse_request(db.get_pool(), notification_bus, &user_id, &friend_id).await {
        Ok(_) => json!({
            "status": 200,
            "message": "Friend request refused"
        }),
        Err(e) => {
            error!("[User] Failed to refuse friend request: {}", e);
            json_error(500, "Failed to refuse friend request")
        }
    }
}
