use crate::{config::ServiceConfig, metrics::app_metrics::AppMetrics, port_utils::find_free_port};
use std::sync::Arc;
...
pub fn get_metrics_port(config: &mut ServiceConfig) -> Option<u16> {
    ...
    if config.metrics_manager_connected == false {
        ...
        if let Some(p) = port {
            ...
            warn!("[Gateway Metrics] No free port found");
        ...
        if port.is_none() {
            warn!("[Gateway Metrics] No metrics port set");
            return None;
        }
    }
    return port;
...
pub async fn start_metrics_server(metrics: Arc<AppMetrics>, config: &mut ServiceConfig) -> bool {
    ...
    let listener = match TcpListener::bind(&format!("0.0.0.0:{}", port)).await {
        Ok(l) => {
            warn!("[Gateway Metrics] Server started on {}", addr);
            l
        }
        Err(e) => {
            warn!("[Gateway Metrics] Failed to bind: {}", e);
            return false;
        }
    };

    info!("[Gateway] Metrics server started on {}", addr);
        ...
        match listener.accept().await {
            Ok((socket, _addr)) => {
                ...
                tokio::spawn(async move {
                    handle_gateway_metrics_request(socket, metrics).await;
                });
            }
            Err(e) => warn!("[Gateway Metrics] Accept error: {}", e),
        }
    }
}

async fn handle_gateway_metrics_request(
    ...
    loop {
        ...
        if n == 0 {
            warn!("[Gateway] Connection closed by client prematurely");
            return;
        }
        ...
        if request.len() > 8192 {
            warn!("[Gateway] Metrics request too large");
            let _ = socket
