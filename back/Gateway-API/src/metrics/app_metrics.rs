...
impl AppMetrics {
    ...
    pub fn gather(&self) -> String {
        ...
        output.push_str(&format!("gateway_errors_total {}\n", snap.total_errors));
        output.push_str(&format!("gateway_latency_p95_ms {}\n", snap.global_p95_ms));
        output.push_str(&format!("gateway_latency_p99_ms {}\n", snap.global_p99_ms));
        ...
        for (name, s) in snap.scopes {
            ...
            output.push_str(&format!(
                "service_errors_total{{service=\"{}\"}} {}\n",
                name, s.errors
            ));
            output.push_str(&format!(
                "service_latency_p95_ms{{service=\"{}\"}} {}\n",
                name, s.p95_ms
            ));
            output.push_str(&format!(
                "service_latency_p99_ms{{service=\"{}\"}} {}\n",
                name, s.p99_ms
            ));
        }
